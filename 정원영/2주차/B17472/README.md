...
1. 섬의 갯수를 구한다. 구하면서 섬위치를 표시한다.
2. 각 섬에서 시작해서, 각자 다른 섬으로의 길을 모두 구한다.
   - 섬개수도 적고 map도 작아서 가능함. 아니었으면 가장 짧은것만 pq에 넣도록 했을 듯
3. 찾아가는건 한 점에서 4방향 잡고 쭉 가보기 -> 나오면 멈춤
4. mst를 구한다.

### 💡복기
- 크루스칼 알고리즘을 썻는데 B형 칠때는 pq 쓰기가 힘들다. -> edge들을 배열에 넣어두고 정렬해서 순회하는 방식으로 하자.
- 프림의 알고리즘도 알아두자
- 프림은 ElogV이고 크루스칼은 ElogE이다. 그래서 간선이 많을 땐 프림, 적을땐 크루스칼을 쓰자.
---
